const filmItemDetailsMarkup = (info) => {
  const {title, "alternative_title": original, "total_rating": rating, director, writers, actors, release, runtime, description} = info;
  return (
...
export const createPopupTemplate = (item) => {
  const {"film_info": info, "user_details": details} = item;
  const filmMarkup = filmItemDetailsMarkup(info);
  return (
